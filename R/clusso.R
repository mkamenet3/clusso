...
#'@param model A string specifying which model to use, Poisson or binomial. For Poisson, specify \code{"poisson"} and both the Poisson and quasi-Poisson model results are returned. For binomial, specify \code{"binomial"} and both the binomial and quasi-binomial model results are returned. For bernoulli, specify \code{"bernoulli"}.
...
clusso <- function(df, expected, observed, timeperiod,id=NULL,covars,x,y,rMax, utm=TRUE, analysis = c("space","spacetime", "both"),model = c("poisson", "binomial", "bernoulli"),maxclust = 11,overdispfloor=TRUE, cv=NULL, collapsetime=FALSE, nsize=NULL, focalcells=NULL,subxy =NULL){
    ...
    if(model=="poisson"){
        ...
        switch(analysis, 
               space = clussoBinom(analysis="space",x, y, rMax,period, expected, observed, id,covars,
                                   Time, utm, maxclust, overdispfloor,cv, collapsetime, nsize, focalcells, subxy ),
...
#'clussoPois
#' @description 
#'This function runs both the space and space-time LASSO poisson model. A separate function (clusso) is the helper function which will have 
...
#'clussoBinom
#' @description 
#'This function runs both the space and space-time LASSO binomial model. A separate function (clusso) is the helper function which will have 
#'flexibility to specify the space or spacetime or both models to be run .
#'@param analysis A string specifying if the spatial (\code{"space"}), spatio-temporal (\code{"spacetime"}), or both spatial and spatio-temporal (\code{"both"}) analysis should be executed.  
#'@param x x coordinates (easting/latitude); if utm coordinates, scale to km.
#'@param y y coordinates (northing/longitude); if utm coordinates, scale to km.
...
clussoBinom <- function(analysis,x,y,rMax, period, expected, observed, id,covars,Time, utm, maxclust,overdispfloor, cv, collapsetime, nsize, focalcells,subxy ){  
    model <- "binomial"
    #browser()
    if(analysis=="space"){
        analysis_name<-"spatial"
...
#' Detect a cluster in space or spacetime using the LASSO: Bernoulli model
#' @title
#'clussoBern
#' @description 
#'This function runs both the space and space-time LASSO Bernoulli model. A separate function (clusso) is the helper function which will have 
#'flexibility to specify the space or spacetime or both models to be run.
#'@param analysis A string specifying if the spatial (\code{"space"}), spatio-temporal (\code{"spacetime"}), or both spatial and spatio-temporal (\code{"both"}) analysis should be executed.  
#'@param x x coordinates (easting/latitude); if utm coordinates, scale to km.
#'@param y y coordinates (northing/longitude); if utm coordinates, scale to km.
...
clussoBern <- function(analysis,x,y,rMax, period, expected, observed, id,covars,Time, utm, maxclust,overdispfloor, cv, collapsetime, nsize){  
    ...
    if(analysis=="spacetime"){
        ...
        rrcolors.p.st <- colormapping(riskratios.p.st,Time, cv, prob=FALSE)
        #COMBINE RISKRATIOS INTO LISTS
        riskratios <- list(riskratios.p.st = riskratios.p.st)
        rrcolors <- list(rrcolors.p.st = rrcolors.p.st)
        return(list(lassoresult.p.st = lassoresult.p.st,
                    riskratios = riskratios,
                    rrcolors = rrcolors,
                    init.vec = vectors))
        ...
        #COMBINE RISKRATIOS INTO LISTS
        riskratios <- list(riskratios.p.s = riskratios.p.s,
                           riskratios.p.st = riskratios.p.st)
        rrcolors <- list(rrcolors.p.s = rrcolors.p.s,
                         rrcolors.p.st = rrcolors.p.st)
        return(list(lassoresult.p.st = lassoresult.p.st,
                    lassoresult.p.s = lassoresult.p.s,
                    riskratios = riskratios,
                    rrcolors = rrcolors,
