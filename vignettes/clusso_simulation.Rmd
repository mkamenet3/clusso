---
title: "Running *clusso* with Simulations"
...
In this first simulation, we will explore how to use the `clusso` package to perform a simple simulation using the Japanese breast cancer data set. For simplicity, no covariates are included and `nsim=10` for speed and illustration.
...
Since this is a simulation, we must first create a cluster to be found using `clusso`. We create a cluster centered at centroid #1 (`center=1`), with a radius of 18km (`radius=18`), that exists in all 5 time periods (`timeperiod=c(1:5)`), has a risk.ratio of 2 (`risk.ratio=2`), moderate overdispersion (`theta=1000`).
...
Finally, we are ready to perform the simulation using the `clusso_sim()` function.

```{r}
#run clusso_sim and time it
system.time(res <- clusso_sim(clst, x,y,rMax, Time,
...
system.time(res <- clusso_sim(clst, x, y,rMax,Time, nsim,center, radius, risk.ratio, timeperiods, 
...
We can also output the threshold results, just to explore additional diagnostics (still underdevelopment, please [report any bugs](https://github.com/mkamenet3/clusso/issues)!)
...
We also allow for the model to take on covariates, which are specified in the ```clst``` class definition and are then passed to the larger ```clusso_sim()``` function.
...
clst <- toclusso(df, expected = japanbreastcancer$expdeath, observed = japanbreastcancer$death, timeperiod = japanbreastcancer$period, covars=TRUE)
...
clst <- toclusso(japanbreastcancer, expected = japanbreastcancer$expdeath, observed = japanbreastcancer$death, timeperiod = japanbreastcancer$period, covars=FALSE)
