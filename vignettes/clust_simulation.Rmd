...
filename <- paste0("sampleoutput/",sim.i,"singlesim.RData")
save(res, file = filename)
...
#WRITE TO CSV
print(tabn)
write.csv(tabn, file="sampleoutput/tabledetectionsinglesim.csv", row.names=TRUE)
...
clst <- toclust(dframe, expected = dframe$expdeath, observed = dframe$death, timeperiod = dframe$period, covars=TRUE)
