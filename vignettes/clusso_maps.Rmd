...
set.seed(2)
ncounties <- length(unique(wi$NAME))
ntrials <- rnbinom(n=ncounties, mu=1000, size =10)
dat <- cbind.data.frame(id=unique(wi$NAME),ntrials)
dat <- dat %>%
...
To prepare `widat` for use with `clusso()`, we convert `time` and `NAME` to factors:

```{r}
widat$time <- as.factor(widat$time)
widat$NAME <- as.factor(widat$NAME)
```
...
```{r}
coords <- as.data.frame(st_coordinates(st_centroid(widat)))
head(coords)
```
...
To perform the regularized spatial and spatio-temporal cluster detection using the Lasso penalty, we use the function `clusso()`. In the first argument, we set the name of the dataframe, `df=widat`. In the second argument we set `expected` counts equal to the total number of trials in the `widat` dataframe (`expected = ntrials`). In the third argument, we set the observed counts equal to the total number of cases in each county in the `widat` dataframe (`observed = ncases`). In the fourth argument, we set the timeperiod equal to the name of the time periods in the `widat` dataframe (`timeperiod = time`).  In this example, we do not want to include other covariates as un-penalized terms in the model so we set `covars = FALSE`. 
...
Next, we combine the estimated relative risks (`resclusso$lassoresult.p.s$E.qbic`), county names (`widat$NAME`), and mapped colors (`cluster_ix`) into the dataframe `rr.bic`. We map the county names onto the colors (`names(cluster_ix) <- rr.bic$county`).
