...
```{r}
head(calcexpected(jbc$death, factor(jbc$period), factor(jbc$id), family="poisson"))
```

To perform the LASSO-based cluster detection, we use the function `clusso()`. 
...
```{r}
system.time(resreal <- clusso(clst, x,y, rMax, Time, utm=TRUE, analysis="both", model="poisson",maxclust=11))
