...
utmJapan <- read.csv("../data/utmJapan.csv")
```
...
system.time(res<- clusso(clst, x,y, rMax, Time, utm=TRUE, analysis="both", cv=cv))
...
```{r}
clst <- toclusso(df, expected = expdeath, observed = death, timeperiod = period, covars=TRUE)
system.time(res <- clusso(clst, x,y, rMax, Time, utm=TRUE, analysis="both"))
