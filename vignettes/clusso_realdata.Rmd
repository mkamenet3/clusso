...
system.time(resreal <- clusso(df=jbc, expected = expdeath, observed=death,timeperiod = factor(period), covars=FALSE,
...
```{r}
clussoplot(rescv, analysis="both", Time=5, cv=TRUE)
```
...
```{r}
exp(rescovars$lassoresult.qp.st$lasso$beta[-(1:66870), rescovars$lassoresult.qp.st$selections$select.qbic])
```
...
```{r}
clussoplot(rescovars, analysis="both", Time=5)
