...
system.time(res <- clust(clst, covars = NULL, x,y, rMax, Time, utm=TRUE, byrow=TRUE, space="both", floor, cv))
...
floor <- TRUE
cv <- 10 #how many folds
clst <- toclust(df, expected = df$expdeath, observed = df$death, timeperiods = df$period, covars=TRUE)

system.time(res <- clust(clst, x,y, rMax, Time, utm=TRUE, byrow=TRUE, space="both", floor, cv))
