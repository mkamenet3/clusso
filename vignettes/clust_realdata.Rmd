...
The fourth argument is the number of time periods measurements were taken during (`Time`, or 5 in our data). **TODO: address missing data; make this unnecessary if analysis="space" GitHub Issue #50**

The fifth argument species that our coordinates are indeed UTM. The default is lat/lon coordinates.

The sixth argument is `analysis`. The only current option is `both`, which will perform both the spatial and spatio-temporal analyses. **TODO: Allow user specify type of analysis**
...
```{r}
system.time(resreal <- clust(clst, x,y, rMax, Time, utm=TRUE, analysis="space", maxclust=10))
names(resreal)
```
...
```{r}
clustpretty(resreal, analysis="space", clusteridentify=FALSE)

names(clustpretty(resreal, analysis="space", clusteridentify=TRUE))
clustpretty(resreal, analysis="space", clusteridentify=TRUE)

clustpretty(resreal, analysis="space", clusteridentify=TRUE, clusterRR = 1.05)
clustpretty(resreal, analysis="space", clusteridentify=TRUE, clusterRR = "background")
```
