...
clust.sim.all.both <- function(x, y, rMax, period, expected, observed, Time, nsim, center, radius, risk.ratio, 
    ...
    lassoresult.qp.s <- spacetime.lasso.sim(clusters, vectors.sim.s, 1, spacetime=FALSE, pois=FALSE, nsim, YSIM.s)
    message("RUNNING: SPACE-ONLY POISSON")
    lassoresult.p.s <- spacetime.lasso.sim(clusters, vectors.sim.s, 1, spacetime=FALSE, pois=TRUE, nsim, YSIM.s)
    ...
    #DETECTION
    ##QP - Space
    set <- detect.set(lassoresult.qp.s, vectors.sim.s, as.matrix(rr[,timeperiod[1]]), 1, x, y, rMax, center, radius)
    ...
    detect.out.qp.s <- (matrix(unlist(detect.qp.s),ncol=3, byrow=TRUE, 
                               dimnames = list(c(paste0("incluster.any.", threshold[1]),
                                                 ...
                                                 paste0("trueclusterdetect.",threshold[2])),c("aic","aicc","bic"))))
    ##P - Space
    set <- detect.set(lassoresult.p.s, vectors.sim.s, as.matrix(rr[,timeperiod[1]]), 1, x, y, rMax, center, radius)
