library("testthat")
library("MASS")
context("Simulation of clussoer lasso - helper function")

test_that("All parameters are correctly specified and trouble-shooted in clusso_sim", {
    #set up
    ...
    clst <- toclusso(df, expected = df$expected, observed = df$observed, timeperiod = df$timeperiods, covars = FALSE)
    
    #testing error response if clst object not provided
    expect_error(clusso_sim(x_utm, y_utm, r.Max, Time, nsim, center, radius, risk.ratio, timeperiod, utm=TRUE, byrow=TRUE,
                           threshold = 0.5,
                           space = "both", theta, nullmod = NULL, overdispfloor=FALSE))
    #testing error response if clst object not of class clst
    expect_error(clusso_sim(df, x_utm, y_utm, r.Max, Time, nsim, center, radius, risk.ratio, timeperiod, utm=TRUE, byrow=TRUE,
                           threshold = 0.5,
                           space = "both", theta, nullmod = NULL, overdispfloor=FALSE))
})
