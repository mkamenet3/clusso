library("testthat")
library("Matrix")
context("In/out clussoer detection works correctly")
...
test_that("All parameters are correctly specified and trouble-shooted in clusso_sim", {
    ...
    rr[5:10,2:4] <- risk.ratio #create fake clussoer
    
    #create fake coordinates
    set.seed(1)
    x <- rnorm(n, mean = 358.1, sd=33.8)
    y <- rnorm(n, mean = 4016, sd = 32.7)
    clussoers <- clussoers2df(x,y,rMax, utm=TRUE, length(x))
    n <- length(unique(clussoers$center))
    numCenters <- n
    sparseMAT <- spacetimeMat(clussoers, numCenters, Time)
    ...
    moo <- prob_clussoeroverlap(sparseMAT,lassoresult, rr, risk.ratio, x, y, rMax, nsim,Time,thresh)
